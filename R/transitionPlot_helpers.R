...
prTpPlotBox <- function(bx, 
	...
	if (is.na(prop)){
		...
		if (bx_txt != ""){
			bx_grob <- prTpGetBoxSizedTextGrob(bx, txt=bx_txt,
					txt_clr = txt_clr,
					txt_cex = cex)
			if (!is.null(bx_grob))
				grid.draw(bx_grob)
...
#'
#' @keywords internal
prTpGetBoxSizedTextGrob <- function(bx, txt,
		txt_clr,
		txt_cex,
		force_cex = FALSE,
		...){
	...
	bx_width <- convertX(grobWidth(bx_grob), "npc", valueOnly=TRUE)
	...
	if (.9 < bx_height | .9 < bx_width){
		new_cex <- min(txt_cex * .9 / (bx_height), txt_cex * .9 / (bx_width))
...
prTpPlotArrows <- function(type,
	...
	bx_left <- prTpGetBoxPositions(no=box_row, colNum = colNum,
	...
	if (length(box_clrs_right == 1)) {
		box_clrs_right <- rep(box_clrs_right, times = length(transition_flow[box_row,]))
	}
	...
	for (flow in order(transition_flow[box_row,])){
		if (transition_flow[box_row,flow] > 0){
			...
			a_l <- (box_width/4)
			x_ctrl_points <- c(bx_left$right, colNum * 1/totalColNum , colNum * 1/totalColNum, bx_right$left)
			...
			if (type=="grid"){
				...
				if (type=="simple"){
					...
					if (length(box_clrs_right) > 1){
						# Invert order as that is the fill order
						current_grdt_clr <- prTpGetColors(colors = box_clrs_right[flow],
								proportion = 1-transition_arrow_props[box_row, flow],
								space = color_bar_subspace)
					}else{
						current_grdt_clr <- box_clrs_right[flow]
					}
					bz <- bezierArrowGradient2sided(x=x_ctrl_points,
							y=y_ctrl_points,
							width=adjusted_lwd,
							arrow=list(length=a_l, base=a_width),
							clr=current_arrow_clr,
							...
							grdt_start_prop = .7,
							grdt_decrease_prop = .3,
							grdt_clr_left = current_grdt_clr,
							grdt_clr_right = box_clr)
					grid.draw(bz)
...
prTpPlotBoxes <- function (colNum,
	...
	for(i in overlap_order){
		if (prop_start_sizes[i] > 0){
			...
			if(colNum == 1) {
				bx_left <- prTpGetBoxPositions(no=i, colNum = colNum,
						totalColNum = totalColNum,
		...
		if (prop_end_sizes[i] > 0){
			bx_right <- prTpGetBoxPositions(no=i, colNum = colNum + 1,
					totalColNum = totalColNum,	
					transitions=transition_flow[,i],
					prop_sizes = prop_end_sizes,
					tot_spacing=tot_spacing,
...
#' @param colNum the box column number
#' @param totalColNum the total number of box columns
#' @param transitions The flows to or from
#' @param prop_sizes The size of the start boxes
...
#'
#' @keywords internal
prTpGetBoxPositions <- function (no, colNum,
		totalColNum,
		transitions,
		prop_sizes,
		tot_spacing,
		box_width){
	no_boxes <- length(prop_sizes)
	empty_boxes <- sum(prop_sizes==0)
	
	# Calculate basics
	space <- tot_spacing/(no_boxes-1-empty_boxes)

	# Basic box vars
	ret <- list(height = (1-tot_spacing)*prop_sizes[no],	
			width = box_width)
	
	if (no == 1){
		ret$top <- 1
	}else{
		ret$top <- 1 - sum(prop_sizes[1:(no-1)]) * (1-tot_spacing) -
				space*(no-1)
	}
	...
	arrowWidth <- (1 - totalColNum * box_width) / (totalColNum - 1)
	ret$left <- (colNum - 1) * (arrowWidth + box_width)
	ret$right <- (colNum - 1) * (arrowWidth + box_width) + box_width
