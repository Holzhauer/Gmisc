...
#' grid.draw(arrowGrob)
#' @export
bezierArrowGradient2sided <- function(
  width = .05,
  clr = "#000000",
  default.units = "npc",
  align_2_axis = TRUE,
  ...
  grdt_start_prop = .4,
  grdt_clr_prop = .7,
  grdt_line_width = unit(0, "mm"),
  grdt_clr_left  = "#2F4F2F",
  grdt_clr_right = "#2F4F2F",
  vp = NULL,
  gp = gpar(),
  ...){
  ...
  pg <- bezierArrowSmpl(width = width, default.units = default.units,
    clr = grdt_clr_right, align_2_axis = align_2_axis,
    gp = gp, vp = vp, ...)
  ...
  g_clrs <- colorRampPalette(colors=c(grdt_clr_right, grdt_clr_left))(clr_length)
  ...
  if (start_decrease > 1){
    ...
    # It can be either x or y that is closer to the starting point
    ...
    if (length(lines$left$x) > 0){
      gradient_pg <- polygonGrob(y=unit.c(lines$left$y,
        ...
        x=unit.c(lines$left$x,
          ...
          rev(lines$right$x)),
        default.units=default.units,
        gp=gpar(fill=grdt_clr_left, col=grdt_clr_left),
        vp = vp)
